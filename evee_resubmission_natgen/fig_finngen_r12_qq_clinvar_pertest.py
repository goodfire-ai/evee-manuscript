#!/usr/bin/env python3
"""Per-test QQ plot: ClinVar-labeled FinnGen R12 variants, AF-matched.

Each point is one variant-phenotype association test (no aggregation).
Reads artifacts/finngen_r12_qq_clinvar_pertest.feather.
...
Run:
    uv run python evee_resubmission_natgen/fig_finngen_r12_qq_clinvar_pertest.py
"""
...
ARTIFACT = ROOT / "artifacts" / "finngen_r12_qq_clinvar_pertest.feather"
OUT      = Path(__file__).with_name("fig_finngen_r12_qq_clinvar_pertest.png")
OUT_SVG  = Path(__file__).with_name("fig_finngen_r12_qq_clinvar_pertest.svg")
...
def main():
    if not ARTIFACT.exists():
        raise FileNotFoundError(
            ...
            "Run: scripts/prepare/prepare_finngen_r12_qq_clinvar_pertest.py"
        )
    ...
    ax.set_title(
        f"Per-test QQ (all variant×phenotype pairs)\n"
        f"N_path={np_:,}  N_comp={nc:,} (sampled)  N_strict={ns:,} (sampled)"
    )
