#!/usr/bin/env python3
"""
Auto-interpretation quality bar charts — context ablation evaluation.

Panel 1 (Figure 3d): composite score grouped bar chart (3 models x 5 configs)
Panel 2 (supplement): per-axis breakdown (mechanism, bio accuracy, specificity)

Input:  artifacts/context_ablation_eval.feather
Output: figures/figure3/fig3d_interp_quality.{png,pdf}
...
def plot(ax):
    ...
    df = _load_data()
    _plot_barchart(df, "composite", "", ax, ylabel="Composite Score")
...
def main():
    ...
    plot(ax)
    fig.tight_layout()
    save_figure(fig, PANELS / "fig3d_interp_quality")
    print("Saved: fig3d_interp_quality")
