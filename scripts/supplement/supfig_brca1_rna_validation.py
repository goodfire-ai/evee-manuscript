#!/usr/bin/env python3
"""
Supplementary Figure — BRCA1 rs80358099 splice-acceptor prediction validated by RNA-seq.
...
Output: figures/supplement/supfig_brca1_rna_validation.{png,pdf}

Run:
    uv run python scripts/supplement/supfig_brca1_rna_validation.py
"""
import json
import sys
...
OUT_STEM         = ROOT / "figures" / "supplement" / "supfig_brca1_rna_validation"
...
def main():
    ...
    for ti, (head, label) in enumerate(TRACK_HEADS):
        ...
        for x_pos, delta in pts:
            ...
            deltas.append(delta)
            widths.append(max(bp_to_disp_width(x_pos), 0.5))
        if positions:
    ...
    for name, rlo, rhi, dlo, dhi in SEGMENTS:
        if "intron" in name:
            ...
            for x in np.linspace(dlo, dhi, 6)[1:-1]:
                ax_mid.text(x, 0, "◀", fontsize=7, ha="center", va="center",
                            color="#bbb", zorder=4)
