#!/usr/bin/env python3
"""
Prepare artifact for FinnGen R12 ACMG resubmission figure.

Input:  /mnt/home/ryo/finngen_r12_actionable.tsv  (canonical source)
Output: artifacts/finngen_resubmission.feather

Extracts the 6 ClinVar resubmission candidates, parses mixed text/numeric
ACMG criteria columns, and writes a clean feather file for the figure script.

Run:
    uv run python scripts/prepare/finngen_resubmission_artifact.py
"""
import re
import sys
...
OUT  = ROOT / "artifacts" / "finngen_resubmission.feather"
...
def main():
    ...
    df = df[df["variant_id"].isin(RESUBMISSION_IDS)].copy()
